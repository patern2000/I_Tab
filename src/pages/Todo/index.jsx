...
const move = (listGroup, source, destination) => {
    ...
    destTodoList.splice(destination.index, 0, removed);// 添加元素
...
export default function Todo() {
    ...
    const groupTemp = {// 防止todoGroup中确实todo doing done 导致页面渲染出问题
        todo: [...(todoGroup['todo'] || [])],
    ...
    const handleSwitch = (event, theme) => {
        const todoCanvas = toCanvas(targetRef.current);//promise对象
    ...
    const onDragEnd = (result) => {
        ...
        const { droppableId: sId } = source;// 源可拖拽区域 {index: 0, droppableId: 'droppable1'} droppableId可拖拽区域的第index个Draggable组件
        const { droppableId: dId } = destination;
        //拖拽source 和 destination 同源  
        if (!isBlank(sId) && !isBlank(dId) && sId === dId) {// sid 就是字符串"todo"
                    ...
                    >
                        <FlexRowAuto>
                            <DragDropContext onDragEnd={onDragEnd}>
                                {_.map(groupTemp, (value, key) => {// key: todo doing done
                                    return (
                                        // 每一列都有一个 title和可拖拽区域droppable
                                        <FlexColumn
                                            key={key}
                                            style={
                                                fullScreen
                                        ...
                                        >
                                            ...
                                            <Droppable droppableId={key} key={key}>
                                                {(droppableProvided, snapshot) => (// 可拖住区域的标识设置为droppabledId： todo doing done
                                                    <div
